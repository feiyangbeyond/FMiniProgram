...
async function getsaveurl(longurl) {
    var url = `https://api.oddfar.com/g/run/q.php?url=${longurl}`
    try {
        var options = {
            uri: url
        };
...
async function parse(res_json) {
    ...
    if (playwm) {
        ...
        //var shorturl = await getshorturl(realAddress);
        return {
            'code': 1,
            'tryCount': try_count,
            'tested': tested,
...
exports.main = async (event, context) => {
    ...
    if(event.save=="yes"){        
        var saveurl = await getsaveurl(event.url);
        return saveurl
    }else{
